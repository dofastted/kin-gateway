...
export function buildDashboard({ cfg, accountQuota, stickyRouter, routingConfig, stats, requestLog = null, poolScheduler = null, proxyPool = null }) {
  const active = getActiveVmId(cfg.paths.project)
  const pool = poolScheduler?.snapshot?.() || {}
  const poolSnap = snapshotPool(proxyPool)
  const vms = listVms(cfg.paths.project).map((v) => enrichVm(v, accountQuota, active, { routingConfig, pool, poolSnap }))
...
export function buildVmList({ cfg, accountQuota, routingConfig = {}, poolScheduler = null, proxyPool = null }) {
  const active = getActiveVmId(cfg.paths.project)
  const pool = poolScheduler?.snapshot?.() || {}
  const poolSnap = snapshotPool(proxyPool)
  const vms = listVms(cfg.paths.project).map((v) => enrichVm(v, accountQuota, active, { routingConfig, pool, poolSnap }))
...
export function buildVmDetail({ cfg, accountQuota, id, routingConfig = {}, poolScheduler = null, requestLog = null, proxyPool = null }) {
  ...
  const pool = poolScheduler?.snapshot?.() || {}
  const poolSnap = snapshotPool(proxyPool)
  const summary = enrichVm(summarizeVm(vm), accountQuota, active, { routingConfig, pool, poolSnap })
