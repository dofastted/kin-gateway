...
/** Panel-safe preview. Never return the raw token. */
export function redactOauthToken(value, keep = 12, tail = 8) {
  const s = String(value || '').trim()
  if (!s) return null
  if (s.length <= keep + tail) return s.slice(0, Math.min(4, s.length)) + '…'
